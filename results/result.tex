...
As mentioned in Section \ref{sec:cost_result}, all strategies and sampling rates finds
the same initial solution, and then posts the respective constraints for each strategy.
All three strategies begin at the same starting point and since the same branching strategy
is used for all strategies (see Section \ref{sec:branch_strategy}), they vary the same
variables in the same way for each branch. This means that e.g the enumerate strategy will
exhaust all possible variations of the initial set of active instructions before exploring
a new set of active instructions. In a similar manner the registers strategy will explore
all possible register allocations of the initial set of active operations before exploring
solutions with a different set of active operations.

Apart from cost, the set of active operations is the first variable to be assigned during
search. This explains the long execution time of the code generator for the schedule
strategy; For each solution found all branching decisions following the cost are
discarded. In contrast the registers strategy can keep the decisions relating to active
operations and which instruction implements which operation.
