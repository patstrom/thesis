...
this buffer overflow vulnerability. By disassembling the executable and analyzing the
...
instruction will then read the now current value at the top of the stack, which is the
address to the \textit{system()} call, and transfer the program control there. The
\textit{system()} call will read the argument in the \textit{rdi} register and execute
the command.
